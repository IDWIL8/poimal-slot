...
import { Download, Edit3, LoaderCircle, LogIn, LogOut, Search, ShieldCheck, Trash2, Users, X } from 'lucide-react'
...
import { adminDeleteBooking, listBookings, moveBooking, saveAdminNotes, updateBooking } from '../services/bookingService'
...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  async function refresh() { setLoading(true); try { setBookings(await listBookings()) } finally { setLoading(false) } }
  useEffect(() => { refresh() }, [])
  ...
  const chartData = useMemo(() => {
    ...
    return [...new Set(slots.map((slot) => slot.dayKey))].map((key) => {
      ...
      return { name: daySlots[0].dayShort, занято: bookings.filter((booking) => daySlots.some((slot) => slot.id === booking.slotId)).length, всего: 3 }
    })
  }, [bookings])
  const bookedWeek1 = bookings.filter((booking) => slotById(booking.slotId)?.week === 1).length
  const bookedWeek2 = bookings.filter((booking) => slotById(booking.slotId)?.week === 2).length

  function exportCSV() {
    const headers = ['Name', 'Team', 'Week', 'Date', 'Time', 'Development goal']
    const lines = bookings.map((booking) => { const slot = slotById(booking.slotId); return [booking.fullName, booking.team, slot?.week, slot?.date, slot?.time, booking.developmentGoal].map((value) => `"${String(value || '').replaceAll('"', '""')}"`).join(',') })
  ...
  async function remove(booking: Booking) { if (!confirm(`Удалить запись ${booking.fullName}?`)) return; await adminDeleteBooking(booking.token); toast.success('Запись удалена'); refresh() }
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-16">
      ...
      <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[
        ['Записей', bookings.length, 'bg-lime'], ['Свободно', 18 - bookings.length, 'bg-sage'], ['Неделя 1', `${bookedWeek1}/9`, 'bg-blue'], ['Неделя 2', `${bookedWeek2}/9`, 'bg-[#f1e3f4]'],
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.4fr_.6fr]">
        ...
        <div className="rounded-[28px] bg-ink p-7 text-white dark:bg-[#090a08]"><Users className="text-lime" /><p className="mt-10 text-5xl font-extrabold tracking-[-.06em]"><AnimatedNumber value={Math.round((bookings.length / 18) * 100)} suffix="%" /></p><p className="mt-2 text-sm font-bold">общая заполняемость</p><div className="mt-6 h-2 overflow-hidden rounded-full bg-white/10"><motion.div initial={{ width: 0 }} animate={{ width: `${bookings.length / 18 * 100}%` }} className="h-full rounded-full bg-lime" /></div><p className="mt-4 text-xs text-white/40">Осталось {18 - bookings.length} слотов из 18</p></div>
      </div>
      <div className="surface mt-5 overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-black/[.06] p-4 dark:border-white/[.07] sm:flex-row"><label className="relative flex-1"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-black/35 dark:text-white/35" /><input value={query} onChange={(e) => setQuery(e.target.value)} className="field !py-3 !pl-11" placeholder="Имя, команда или тема…" /></label><select value={week} onChange={(e) => setWeek(e.target.value)} className="field sm:w-40"><option value="all">Все недели</option><option value="1">Неделя 1</option><option value="2">Неделя 2</option></select><select value={sort} onChange={(e) => setSort(e.target.value as 'date' | 'name')} className="field sm:w-44"><option value="date">По времени</option><option value="name">По имени</option></select></div>
        {loading ? <div className="grid h-52 place-items-center"><LoaderCircle className="animate-spin text-black/30" /></div> : rows.length === 0 ? <div className="p-14 text-center"><p className="text-3xl">🗓️</p><p className="mt-3 text-sm font-extrabold">Ничего не найдено</p></div> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-black/[.025] text-[10px] uppercase tracking-wider text-black/35 dark:bg-white/[.025] dark:text-white/35"><tr><th className="px-5 py-3">Участник</th><th className="px-5 py-3">Время</th><th className="px-5 py-3">Тема</th><th className="px-5 py-3">Статус</th><th className="px-5 py-3" /></tr></thead><tbody>{rows.map((booking) => { const slot = slotById(booking.slotId); return <tr key={booking.token} className="border-t border-black/[.05] dark:border-white/[.06]"><td className="px-5 py-4"><p className="font-extrabold">{booking.fullName}</p><p className="mt-1 text-xs text-black/35 dark:text-white/35">{booking.team}</p></td><td className="px-5 py-4"><p className="text-xs font-bold">{slot?.dayShort} · {slot?.time}</p><p className="mt-1 text-[10px] text-black/35 dark:text-white/35">Неделя {slot?.week}</p></td><td className="max-w-64 px-5 py-4 text-xs text-black/55 dark:text-white/55">{booking.developmentGoal || '—'}</td><td className="px-5 py-4"><span className={`rounded-full px-3 py-1.5 text-[10px] font-extrabold ${booking.notes?.completed ? 'bg-sage text-ink' : 'bg-[#fff0c9] text-ink'}`}>{booking.notes?.completed ? 'Проведена' : 'Запланирована'}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-1"><button onClick={() => setSelected(booking)} className="grid h-9 w-9 place-items-center rounded-full hover:bg-black/5 dark:hover:bg-white/10" aria-label="Редактировать"><Edit3 size={15} /></button><button onClick={() => remove(booking)} className="grid h-9 w-9 place-items-center rounded-full text-red-400 hover:bg-red-500/10" aria-label="Удалить"><Trash2 size={15} /></button></div></td></tr> })}</tbody></table></div>}
      </div>
...
function BookingEditor({ booking, bookings, onClose, onSaved }: { booking: Booking; bookings: Booking[]; onClose: () => void; onSaved: () => void }) {
  ...
  const occupied = new Set(bookings.filter((item) => item.token !== booking.token).map((item) => item.slotId))
  async function save() {
    setBusy(true)
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/35 backdrop-blur-sm" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><motion.aside initial={{ x: 80, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="h-full w-full max-w-xl overflow-y-auto bg-[#fbfaf7] p-6 shadow-2xl dark:bg-[#20211d] sm:p-8"><div className="flex items-start justify-between"><div><p className="eyebrow">Карточка участника</p><h2 className="mt-2 text-2xl font-extrabold">{booking.fullName}</h2></div><button onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full bg-black/5 dark:bg-white/10"><X size={18} /></button></div><div className="mt-7 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold">Имя<input className="field mt-2" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} /></label><label className="text-xs font-bold">Команда<input className="field mt-2" value={form.team} onChange={(e) => setForm({ ...form, team: e.target.value })} /></label></div><label className="mt-4 block text-xs font-bold">Тема<textarea className="field mt-2 resize-none" rows={3} value={form.developmentGoal} onChange={(e) => setForm({ ...form, developmentGoal: e.target.value })} /></label><label className="mt-4 block text-xs font-bold">Слот<select className="field mt-2" value={slotId} onChange={(e) => setSlotId(e.target.value)}>{createSlots().filter((slot) => !occupied.has(slot.id)).map((slot) => <option value={slot.id} key={slot.id}>Неделя {slot.week} · {slot.dayShort} · {slot.time}</option>)}</select></label><div className="my-7 h-px bg-black/[.07] dark:bg-white/10" /><div className="flex items-center justify-between"><div><p className="eyebrow">Личные заметки</p><p className="mt-1 text-[10px] text-black/35 dark:text-white/35">Видны только администратору</p></div><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={notes.completed} onChange={(e) => setNotes({ ...notes, completed: e.target.checked })} /> Встреча проведена</label></div><div className="mt-5 space-y-4">{noteFields.map(([key, label]) => <label key={key} className="block text-xs font-bold">{label}<textarea className="field mt-2 resize-none" rows={2} value={String(notes[key])} onChange={(e) => setNotes({ ...notes, [key]: e.target.value })} /></label>)}<label className="block text-xs font-bold">Оценка прогресса: {notes.progressRating}/5<input type="range" min="1" max="5" value={notes.progressRating} onChange={(e) => setNotes({ ...notes, progressRating: Number(e.target.value) })} className="mt-3 w-full accent-[#7d9842]" /></label></div><button disabled={busy} onClick={save} className="button-primary mt-7 w-full">{busy ? 'Сохраняем…' : 'Сохранить карточку'}</button></motion.aside></div>
}
